...
load_vals = np.array(
    [hub_volume.get(h, 0) for h in all_hubs],
    dtype=float
)
...
for ax, (emb, title) in zip(axes, [
    ...
    if emb.shape[0] == len(all_hubs):
        current_load_vals = load_vals
    else:
        current_load_vals = load_vals[:emb.shape[0]]
    sizes = 20 + (
       current_load_vals /
       (current_load_vals.max() + 1)
    ) * 120
    sc     = ax.scatter(proj[:, 0], proj[:, 1], s=sizes, alpha=0.65,
                        c=current_load_vals, cmap="viridis", edgecolors="none")
    ...
    for hub in top5:
        if hub in hub2idx: 
            i = hub2idx[hub]
            if i < len(proj):
                ax.annotate(hub,proj[i],fontsize=7,fontweight="bold",color=RED if hub in chronic_hubs else "black",xytext=(4, 3),textcoords="offset points")
